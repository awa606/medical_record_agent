...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、每日工作日志、调试报告和版本演进记录。当前版本新增 ASR 会话级 SSE 文件流编排、医生/患者角色校正和项目管理文档体系，不改变核心 ASR/LLM 算法逻辑。
...
- 角色分离：医生/患者发言策略、逐段人工校正、待确认提示和结构化片段。
...
| v0.3 | Role correction | 医生/患者角色分离、ASR 片段策略、逐段人工校正和校正后病历生成。 |
