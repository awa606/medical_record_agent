...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、每日工作日志、调试报告和版本演进记录。当前版本已覆盖 ASR 会话级 SSE 文件流、医生/患者角色校正、医学知识库规则解释和本地 ASR 多引擎评测，FunASR 与 SenseVoice 已在课程样本上完成本机实测，不改变核心 ASR/LLM 算法逻辑。
...
- ASR 对比引擎：
  ...
  - `whisper`：Whisper Base 本地多语种候选，需要系统可用 `ffmpeg`。
...
| v0.5 | 本地模型评测 | 硬件采集、多引擎运行状态、FunASR/SenseVoice 本机实测、边缘端配置建议。 |
