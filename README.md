...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、GitHub Actions、每日工作日志、调试报告和版本演进记录。当前工程口径为 `v1.4.0 candidate`：已完成 `v1.3.7` 仓库真实性收口和服务端说话人角色质量门禁候选实现，`ASRResult.role_quality` 会记录低置信度角色、未映射 speaker、混合语句和人工确认状态；正式音频病历生成不能绕过角色门禁。默认 Docker 配置仍为 `SPEAKER_ROLE_PROVIDER=rules`，本地 LLM 角色分类是可选能力，不是默认路线。
...
| v1.4.0 candidate | 服务端角色质量门禁 | 统一 `SpeakerRoleQualityPolicy`，正式生成和外部 segments API 不能绕过低置信度、未映射 speaker 或混合语句门禁；合并并打 tag 后才作为正式 `v1.4.0`。 |
