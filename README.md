...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、每日工作日志、调试报告和版本演进记录。当前版本新增 ASR 会话级 SSE 文件流编排和项目管理文档体系，不改变核心 ASR/LLM 算法逻辑。

## 目录

- [Project Overview (Medical Record Agent)](#project-overview-medical-record-agent)
- [Version Timeline (v0.1 -> v1.0)](#version-timeline-v01---v10)
- [System Architecture Summary](#system-architecture-summary)
- [How to Run](#how-to-run)
- [Development Workflow (Issues -> Dev -> Debug Logs -> Commit -> PR)](#development-workflow-issues---dev---debug-logs---commit---pr)
- [主要 API](#主要-api)
- [测试](#测试)
- [数据与隐私边界](#数据与隐私边界)

## Project Overview (Medical Record Agent)
...
- 角色分离：医生/患者发言策略、人工校正提示和结构化片段。
- 医学知识推理：候选诊断、安全校验和医生审核边界。
- 本地模型部署：FunASR、Qwen3-ASR 和 Ollama LLM provider 可选接入。
...
## Version Timeline (v0.1 -> v1.0)

| 版本 | 阶段 | 说明 |
| --- | --- | --- |
| v0.1 | Basic ASR pipeline | 音频上传、ASR 转写结果结构化和病历生成入口。 |
| v0.2 | SSE streaming | ASR 会话分段转写、任务状态、步骤和错误信息的实时流式输出。 |
| v0.3 | Role separation | 医生/患者角色分离、ASR 片段策略和人工校正提示。 |
| v0.4 | Medical reasoning | 病历字段、草稿、安全校验和候选诊断。 |
| v1.0 | Deployable system | 本地启动、可选本地模型、日志、版本和 PR 工作流。 |

完整版本说明见 [`docs/version_log.md`](docs/version_log.md)，里程碑目录见 [`versions/`](versions/)。
...
详细架构见 [`docs/architecture.md`](docs/architecture.md)，ASR 文件流说明见 [`docs/asr_sse_file_stream.md`](docs/asr_sse_file_stream.md)，四周迭代计划见 [`docs/four_week_iteration_plan.md`](docs/four_week_iteration_plan.md)，工程规则见 [`docs/engineering_rules.md`](docs/engineering_rules.md)，能力证据见 [`docs/traceability_matrix.md`](docs/traceability_matrix.md)。
...
## Development Workflow (Issues -> Dev -> Debug Logs -> Commit -> PR)
...
- `POST /api/asr/sessions?engine=mock|funasr|qwen3|online`
...
- [`docs/four_week_iteration_plan.md`](docs/four_week_iteration_plan.md)
