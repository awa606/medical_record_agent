...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、每日工作日志、调试报告和版本演进记录。当前版本已覆盖 ASR 会话级 SSE 文件流、医生/患者角色校正、医学知识库规则解释、本地 ASR 多引擎评测、医生端产品化工作台入口和长音频切片 SSE 状态展示，FunASR、SenseVoice 与 Whisper 已完成本机样本实测，不改变核心 ASR/LLM 算法逻辑。
...
| v0.6 | 医生端产品化 | 优化医生工作台实时转写状态、长音频切片进度、角色校正入口和审核导出按钮可用性。 |
