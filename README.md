...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、GitHub Actions、每日工作日志、调试报告和版本演进记录。当前工程口径为 `v1.4.0`：已完成 `v1.3.7` 仓库真实性收口和服务端说话人角色质量门禁，`ASRResult.role_quality` 会记录低置信度角色、未映射 speaker、混合语句和人工确认状态；正式音频病历生成不能绕过角色门禁。默认 Docker 配置仍为 `SPEAKER_ROLE_PROVIDER=rules`，本地 LLM 角色分类是可选能力，不是默认路线。

第一次看项目时，建议先读本 README，再读 [`docs/项目文件夹阅读指南.md`](docs/项目文件夹阅读指南.md)。阅读指南按文件夹说明每个目录的用途、应该看什么、以及不同任务如何验收。

仓库治理规则见 [`docs/repository_governance.md`](docs/repository_governance.md)，当前按单人维护模式要求 PR 和 CI，不要求同账号自审。
...
| v1.3.7 | 工程真实性收口 | README、ROADMAP、Issue、CI 和 PR 口径对齐，不新增业务能力。 |
| v1.4.0 | 服务端角色质量门禁 | 统一 `SpeakerRoleQualityPolicy`，正式生成和外部 segments API 不能绕过低置信度、未映射 speaker 或混合语句门禁。 |

当前唯一迭代路线见 [`ROADMAP.md`](ROADMAP.md)。完整版本说明见 [`docs/版本演进记录.md`](docs/版本演进记录.md)，里程碑目录见 [`versions/`](versions/)；旧计划文档只作为历史归档参考。
...
medical_record_agent/
  ...
  logs/                        每日日志和调试报告
  .github/                     Issue 模板、PR 模板和 GitHub Actions CI
  versions/                    版本归档入口
