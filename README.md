...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、每日工作日志、调试报告和版本演进记录。当前版本已覆盖 ASR 会话级 SSE 文件流、医生/患者角色校正、医学知识库规则解释和本地 ASR 多引擎评测运行器，不改变核心 ASR/LLM 算法逻辑。
...
- 本地模型部署：FunASR、Qwen3-ASR 和 Ollama LLM provider 可选接入，并提供多引擎评测运行器。
...
| v0.5 | 本地模型评测 | 硬件采集、mock 基线、多引擎运行状态、边缘端配置建议。 |
