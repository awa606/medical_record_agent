...
本仓库同时维护工程管理结构：GitHub Issues、PR 模板、每日工作日志、调试报告和版本演进记录。当前重构只新增项目管理与文档体系，不改变核心算法逻辑。
...
- ASR：音频上传、Mock/本地/线上 ASR 转写、CER 与关键词召回评测。
- SSE：任务状态、步骤和调试信息的实时流式输出。
...
| v0.2 | SSE streaming | 任务状态、步骤和错误信息的实时流式输出。 |
...
3. 选择课程样例音频。
4. 选择可用 ASR 引擎。
5. 转写完成后可打开“ASR评测”，输入人工标注和关键词，查看 CER、keyword_recall、recognized、missing。
