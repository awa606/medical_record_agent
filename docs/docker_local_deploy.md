...
- 不包含公网暴露、HTTPS、企业级身份治理、GPU/CUDA 或医院生产部署；系统已有本地登录认证，但不适合公网直接暴露。
...
当前系统已有本地登录认证和上传大小限制，但仍不包含 HTTPS、公网安全加固、企业级身份治理、集中审计和反向代理配置。公网访问应另行设计 HTTPS 终止、反向代理、访问控制和密钥管理。
