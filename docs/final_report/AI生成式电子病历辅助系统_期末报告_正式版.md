...
本项目面向门诊问诊场景，设计并实现了一个 AI 生成式电子病历辅助系统课程 POC 原型。系统支持文本问诊和预录音频两类输入：文本输入直接进入病历 Agent 主流程，音频输入先通过 ASR 转写为统一的 ASRResult，再进入同一条字段抽取、病历草稿生成、安全校验和医生审核流程。项目采用 `Plan-and-Execute + Human-in-the-loop` 智能体设计模式，由 Orchestrator 将任务拆解为字段抽取、草稿生成、安全校验和医生审核等可追踪步骤，并通过 SSE、Task Steps、Agent Trace 和运行日志记录执行过程。系统提供医生端三栏工作台和调试台：医生端用于展示病历字段、对话转写、缺失提醒、候选诊断和安全校验，调试台用于展示 ASRResult、Task、Steps、Safety 和 Agent Trace JSON。为保证课程现场演示稳定，项目保留 MockLLM fallback，并支持可选 Online / Ollama LLM 字段抽取；真实 LLM 异常时自动降级，不破坏主流程。项目明确边界：本系统不是真实临床系统，不接真实患者数据，不接真实 HIS/EMR，AI 只生成病历草稿、候选诊断和安全提醒，最终字段确认和导出必须由医生审核。测试和运行日志表明，系统能够完成 fever clean 文本和 `fever_01.wav + FunASR` 演示主线，并具备可解释、可追踪和可降级的工程特征。
...
门诊问诊过程中，医生需要将自然语言对话整理为结构化电子病历，包括主诉、现病史、伴随症状、既往史、过敏史、查体、候选诊断和处理建议等内容。人工整理病历需要医生在沟通、判断和记录之间频繁切换，容易出现字段遗漏、证据不清、候选诊断未标注等问题。ASR 和生成式 AI 可以辅助把问诊音频或文本转换为结构化草稿，但医疗场景对准确性、可解释性和责任边界有更高要求，不能简单地把输入交给模型并直接输出最终病历。

本项目的目标是实现一个课程 POC 原型：AI 生成式电子病历辅助系统。系统围绕“可追踪、可审核、可降级”的原则设计，支持文本和音频两种输入，最终统一进入病历 Agent 主流程。项目重点不是替代医生诊断，而是展示如何将生成式 AI 放入一个受控的医疗辅助工作流中。

项目目标包括：

- 支持从人工问诊文本生成结构化病历草稿。
- 支持上传预录音频，通过 ASR 转写后生成病历草稿。
- 支持 ASR 评测，展示 CER、keyword_recall、recognized、missing。
- 支持医生端三栏工作台，展示病历字段、对话转写、AI 辅助与安全校验。
- 支持调试台、Agent Trace、Task Steps 和运行日志，证明过程可解释、可追踪。
- 保留 MockLLM fallback，保证课程演示稳定和工程鲁棒性。
- 明确医疗安全边界：AI 只生成草稿和候选诊断，最终确认和导出必须由医生完成。

图 1：系统入口页

【截图待插入：打开 `/static/index.html`，展示项目标题、项目简介、医生端工作台入口和开发调试台入口。】

图 1 系统入口页，提供进入医生端工作台和开发调试台两个入口。
...
1. 从文本生成病历：用户粘贴人工问诊文本，系统生成结构化字段、病历草稿、安全校验和 Agent Trace。
2. 上传预录音频测试转写：用户上传音频并选择 ASR 引擎，仅查看转写结果和 ASR 评测，不进入病历生成。
3. 上传预录音频生成病历：用户上传音频，系统先 ASR 转写，再把 `conversation_text` 送入病历 Agent 主流程。

系统还需要支持以下能力：

- ASR 对比：`mock`、`funasr`、`qwen3`、`online` 统一输出 ASRResult。
- ASR 评测：字符错误率 CER、医学关键词召回率 keyword_recall、recognized、missing。
- LLM provider：默认 MockLLM，支持 Online 和 Ollama 字段抽取，失败时 fallback。
- 医生端工作台：左栏字段、中栏转写、右栏 AI 辅助与安全校验。
- 调试台：展示 ASRResult、Task、Steps、Safety、Agent Trace JSON。
- 运行日志：根据 `task_id` 和 `audio_id` 生成演示运行记录。

### 2.2 非功能需求

| 需求 | 说明 |
| --- | --- |
| 稳定性 | 课程现场演示不能完全依赖外部网络和真实模型输出，必须保留 MockLLM fallback |
| 可追踪性 | 每个任务步骤都要记录状态、输入输出快照、错误和耗时 |
| 可解释性 | 字段保留证据片段，候选诊断保留来源，Agent Trace 展示决策过程 |
| 安全性 | 医生审核前不得自动导出最终病历，不得编造原文未出现事实 |
| 隐私保护 | 不接真实患者数据，不提交真实 API Key、数据库、模型缓存和大体积音视频 |
| 可扩展性 | ASR 和 LLM 使用 Adapter / Factory，便于替换引擎而不改变主流程 |

图 2：医生端无任务引导卡片

【截图待插入：打开 `/static/doctor.html` 的无任务状态，展示“开始一次病历生成”引导卡片和文本 / 音频输入按钮。】

图 2 医生端无任务引导卡片，提示医生选择文本导入或上传问诊音频开始。
...
【截图待插入：建议将下方 Mermaid 架构图渲染为图片后插入，展示文本 / 音频输入、ASRResult、Orchestrator、字段抽取、草稿生成、安全校验、医生审核和审计反馈。】

图 3 系统总体架构图，展示系统从输入感知到医生审核和审计反馈的 Agent 闭环。

备用 Mermaid 源码如下，正式 Word 中建议替换为渲染后的图片：
...
架构设计的核心思想是把输入、工具调用、模型调用、医生审核和审计记录分离。ASR 的输出统一为 ASRResult，LLM 的输出统一进入 Pydantic Schema，前端只根据状态渲染医生工作台，不直接承担决策逻辑。

## 4. 智能体设计模式

本项目采用 `Plan-and-Execute + Human-in-the-loop` 设计模式。它不是一次性调用模型生成病历，而是把病历生成拆解为多个可观察、可失败、可降级、可审核的步骤。
...
这种设计使课程演示重点从“模型生成了什么”转向“系统如何把生成式 AI 放进可控工作流”。医生不是临时兜底，而是流程设计的一部分。

图 7：调试模式 Agent Trace

【截图待插入：在 `/static/doctor.html` 切换到调试模式，展示右栏 Agent Trace 中的 `agent_mode`、`plan`、`executed_steps` 和 `decision`。】

图 7 调试模式下的 Agent Trace，展示 Plan-and-Execute 与 Human-in-the-loop 决策轨迹。
...
【截图待插入：展示 `fever_01.wav` 经 FunASR 转写后的中栏 conversation_text、分段文本或角色校正提醒。】

图 5 对话转写区域，音频经 ASR 转为 conversation_text 后进入病历 Agent。
...
【截图待插入：打开 `/static/doctor.html` 医生模式，展示左栏病历字段、中栏对话转写、右栏 AI 辅助与安全校验。】

图 4 医生模式三栏工作台，主页面隐藏调试细节，突出医生审核流程。

图 6：AI 辅助与安全校验区域

【截图待插入：展示右栏缺失项提醒、候选诊断、安全校验、红黄风险项和操作区。】

图 6 AI 辅助与安全校验区域，红黄风险项提示医生优先处理。

图 8：fever_01.wav 生成病历后的病历字段

【截图待插入：上传 `fever_01.wav` 并完成病历生成后，截取左栏主诉、现病史、伴随症状、查体和候选诊断字段卡片。】

图 8 fever_01.wav 生成病历后的病历字段卡片，字段状态和证据用于医生核对。
...
【截图待插入：打开 `/static/debug.html`，展示 Task JSON、Steps JSON 和 Safety JSON。】

图 9 调试台 Task、Steps 和 Safety JSON，用于证明系统过程可追踪。

图 10：运行日志 fever_01_final_demo

【截图待插入：打开 `docs/dev_logs/runs/2026-06-20_fever_01_final_demo.md`，展示运行时间、ASRResult 摘要、Agent Trace 和安全校验摘要。】

图 10 fever_01_final_demo 运行日志，记录 ASRResult、Agent Trace、任务状态和安全校验摘要。
...
- `2026-06-20_scoring_final_report_materials.md`：评分看板与报告初稿整理。
- `2026-06-20_fix_save_run_log_timezone.md`：运行日志脚本时区修复。
...
- 真实 API Key
- 真实患者数据
- 其他课程目录，例如 `../../Intelligent product design/` 或 `../homework/`

## 附录 A：评分细则对照表

| 评分项 | 满分要求 | 当前证据 |
| --- | --- | --- |
| 智能体设计模式 | 能说明 Agent 的感知、计划、执行、反馈和医生审核边界 | Orchestrator、Agent Trace、SSE、Task Steps |
| 决策系统设计 | 能展示输入分流、模型选择、缺失字段、安全校验和导出门禁 | decision_system、Prompt 链、Schema、SafetyCheck |
| 伦理合规设计 | 能说明隐私、医疗安全、防注入、审计追踪和局限性 | ethics_compliance、README、审计日志、医生审核 |
| 演示流畅度 | 能稳定展示文本、音频、ASR 评测、Agent Trace 和运行日志 | doctor.html、debug.html、fever_01 运行日志 |
...
| 文件 | 用途 |
| --- | --- |
| `app/agents/medical_record_orchestrator.py` | Agent 编排、状态流转、步骤记录和降级 |
| `app/services/agent_trace.py` | 组装 Agent Trace 和导出决策 |
| `app/schemas/medical_record.py` | 病历字段、候选诊断、安全校验 Schema |
| `app/schemas/asr.py` | ASRResult、ASRSegment、评测结果 Schema |
| `app/services/asr/factory.py` | ASR 引擎选择 |
| `app/services/asr/funasr_engine.py` | FunASR baseline |
| `app/services/llm/factory.py` | LLM provider 选择与状态检查 |
| `app/services/llm/llm_record_generator.py` | 字段抽取、JSON 校验与 fallback |
| `app/prompts/medical_record_prompts.py` | Prompt 链和真实 LLM 接入契约 |
| `app/db/sqlite.py` | task、task_step、audit_log |
| `static/doctor.html`、`static/doctor.js` | 医生端工作台 |
| `static/debug.html` | 调试台 |
| `scripts/save_run_log.py` | 运行日志生成 |

## 附录 D：截图清单

详细截图规划见 `docs/final_report/截图清单.md`。
...
2. 医生端无任务引导卡片。
3. 医生模式三栏工作台。
4. `fever_01.wav` 生成病历后的字段卡片。
5. 对话转写区域。
6. AI 辅助与安全校验区域。
7. 调试模式 Agent Trace。
8. debug.html Task / Steps / Safety JSON。
9. `fever_01_final_demo` 运行日志。
10. 评分进度看板。
...
| 日志 | 说明 |
| --- | --- |
| `docs/dev_logs/TEMPLATE.md` | 开发日志模板 |
| `docs/dev_logs/DEVELOPMENT_RULES.md` | 开发日志与提交规范 |
| `docs/dev_logs/V0.1_text_agent_skeleton.md` | 文本版 Agent 骨架 |
| `docs/dev_logs/V0.2_mock_asr_three_entries.md` | Mock ASR 与三入口 |
| `docs/dev_logs/V0.3_funasr_asr_evaluation.md` | FunASR 与 ASR 评测 |
| `docs/dev_logs/V0.3.1_fever_field_rules.md` | fever 字段抽取规则 |
| `docs/dev_logs/V0.4_doctor_debug_index_split.md` | 三页面拆分 |
| `docs/dev_logs/V0.4.3_doctor_workbench_ui.md` | 医生工作台 UI 优化 |
| `docs/dev_logs/2026-06-10_issue_33_agent_trace.md` | Agent Trace 可见化 |
| `docs/dev_logs/2026-06-10_llm_integration.md` | LLM Adapter 和 MockLLM fallback |
| `docs/dev_logs/2026-06-11_issue_36_asr_llm_status.md` | ASR / LLM 状态区分 |
| `docs/dev_logs/2026-06-11_issue_37_save_draft_run_context.md` | 草稿保存和运行日志命令 |
| `docs/dev_logs/2026-06-15_issue_38_doctor_guidance_demo_mode.md` | 医生端引导和演示模式 |
| `docs/dev_logs/2026-06-20_scoring_final_report_materials.md` | 评分看板与报告初稿 |
| `docs/dev_logs/2026-06-20_final_report_formalization.md` | 正式报告整理 |
