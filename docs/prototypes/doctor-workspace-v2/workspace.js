...
(() => {
  ...
  function renderTranscript() {
    const pending = ["empty", "recording"].includes(state.mode);
    $("transcript-content").innerHTML = pending
      ? '<div class="ml-transcript-empty"><h3>对话原文显示在这里</h3><p>开始录音、上传或输入文本后，在这里核对说话人和原文。</p><p>当前为匿名样稿，不执行真实采音。</p></div>'
      : transcripts
          .map(
            ([speaker, time, text], i) =>
              `<section class="ml-speech" id="utterance-${i}"><header><span class="ml-speaker ${speaker === "患者" ? "patient" : ""}">${speaker}</span><time>${time}</time></header><p>${escape(text)}</p></section>`,
          )
          .join("");
  }
  function renderSummary() {
    $("panel-title").textContent = "临床提示";
    $("panel-kicker").textContent = "仅供医生核对";
    $("close-panel").hidden = true;
    $("panel-content").innerHTML =
      `<section class="ml-summary-card"><span class="ml-tag">核对提醒 · 示例</span><h3>过敏史为否认</h3><p>“没有花生过敏”应保留为否认，不应写成存在过敏。</p><button data-source="allergy_history">核对原文证据</button></section><section class="ml-summary-card"><h3>待补充信息</h3><ul><li>既往病史</li><li>查体结果</li><li>已用药情况</li></ul><p class="ml-panel-caption">未提及不等于无异常；由医生补充。</p></section><section class="ml-summary-card"><h3>临床参考依据</h3><p>查看相关规范、来源与版本，内容仅供参考。</p><button data-panel="knowledge">查依据</button></section><div class="ml-safe-note">不自动诊断或处方。医生审核后，病历才能进入导出环节。</div>`;
  }
  function panelSemantics() {
    ...
    for (const selector of [
      ...
      document.querySelector(selector).inert = overlay;
  }
  ...
  $("listen-transcript").addEventListener("click", () =>
    notify("试听位置示意：样稿未载入音频"),
  );
  resetValues();
  render();
  renderSummary();
