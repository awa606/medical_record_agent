...
状态：**AWAITING USER REVIEW**。用户认为V2字体过大、冗余内容多，要求参考真实医院系统。用户要求字号/边界更清楚，并按医生录音问诊、再看病历的顺序使用。本版病历正文17px、转写16px、按钮14px、辅助信息13px，患者名22px、就诊/流程16px、栏标题18px；保留三个业务区域，未接生产接口。
...
用户明确认可具体V3.3布局后才允许固定截图基线并接业务。尚未获批时图片回归拒绝；自动测试不能替代用户认可。真实三路径和离线恢复仍未执行；5.1保持BACKLOG，2626不变。
