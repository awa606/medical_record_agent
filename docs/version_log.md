...
| v0.3 | Role correction | 建立医生/患者角色分离、ASR 角色策略、逐段人工校正和校正后病历生成。 | `versions/v0.3_role_separation/` |
...
当前代码已覆盖 `v0.1` 到 `v0.4` 的主要工程能力，并在 `v0.2.1` 增加 ASR 会话级 SSE 文件流转写，在 `v0.3` 增加医生/患者角色校正闭环。`v1.0` 的重点是部署说明、版本证据、日志纪律、模型评测和协作流程。
