...
FORM_SPECS = [
    ...
    ("04", "每日任务记录表_Day3-4", "第1周：核心模块打通"),
    ("05", "系统架构设计表_Day5", "第1周：系统架构设计"),
    ("06", "第1周评分表_POC-Prototype", "第1周：技术打通"),
    ("07", "每日任务记录表_Day6-7", "第2周：系统集成"),
    ("08", "每日任务记录表_Day8-9", "第2周：核心功能实现"),
    ("09", "POC完成检查表_Day10", "第2周：端到端演示"),
    ("10", "第2周评分表_POC-MVP", "第2周：POC达成"),
    ("11", "每日任务记录表_Day11-12", "第3周：性能优化"),
    ("12", "每日任务记录表_Day13-14", "第3周：稳定性测试"),
    ("13", "问题跟踪表_Bug_List_Day15", "第3周：测试复盘"),
    ("14", "系统测试记录表_Day15", "第3周：测试复盘"),
    ("15", "第3周评分表_POC-Refine", "第3周：优化验证"),
    ("16", "每日任务记录表_Day16-17", "第4周：文档完善"),
    ("17", "EVT初级检查表_Day18", "第4周：系统封版"),
    ("18", "第4周评分表_POC交付", "第4周：预答辩"),
    ("19", "综合设计3评分表_总评_Day20", "第4周：终答辩"),
    ("20", "EVT加分表_Day20", "第4周：终答辩"),
]
...
def build_form_doc(code: str, name: str, stage: str, template: bool = False) -> Document:
    ...
    if "个人贡献" in name:
        ...
        add_score_form(doc, "第3周评分表：POC-Refine", "20", [
            ("一、系统稳定性（8分）", "待完成 30-60min 长音频和连续运行测试。"),
...
def build_index_doc(template: bool = True) -> Document:
    ...
    doc.add_paragraph("来源：report/综合设计3学生成果手册.docx")
...
def main() -> None:
    ...
    for folder in [TEMPLATE_DIR, HOMEWORK_DIR / "individual_forms"]:
    ...
    source = REPORT_DIR / "综合设计3学生成果手册.docx"
    if source.exists():
        shutil.copy2(source, REPORT_DIR / "学术成果手册_源文件备份.docx")
