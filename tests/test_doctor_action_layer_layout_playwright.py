...
def _segments(count: int) -> list[dict[str, object]]:
    ...
    for index in range(count):
        ...
        text = (
            f"Segment {index + 1}: please describe symptoms duration associated signs "
            "and previous treatment. layout overflow verification text repeated."
            if role == "doctor"
            else f"Segment {index + 1}: I have fever cough chest tightness and dyspnea. "
            "I took ibuprofen yesterday but symptoms repeated."
        )
        rows.append(
            {
